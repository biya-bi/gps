...
class InstallRequirement(object):
    ...
    def __init__(self, req, comes_from, source_dir=None, editable=False,
        ...
        # have been created. Cf _correct_build_product method.
    ...
    def __str__(self):
        ...
        if self.satisfied_by is not None:
            s += ' in %s' % display_path(self.satisfied_by.product)
        if self.comes_from:
    ...
    def build_product(self, build_dir):
        ...
        if self.req is None:
            ...
            # to fix it via _correct_build_product
    ...
    def _correct_build_product(self):
        """Move self._temp_build_dir to self._ideal_build_dir/self.req.name

        For some requirements (e.g. a path to a directory), the name of the
        package is not available until we run egg_info, so the build_product
        ...
        old_product = self._temp_build_dir
        self._temp_build_dir = None
        new_product = self.build_product(self._ideal_build_dir)
        if os.path.exists(new_product):
            raise InstallationError(
                'A package already exists in %s; please remove it to continue'
                % display_path(new_product))
        logger.debug(
            'Moving package %s from %s to new product %s',
            self, display_path(old_product), display_path(new_product),
        )
        shutil.move(old_product, new_product)
        self._temp_build_dir = new_product
        self._ideal_build_dir = None
        self.source_dir = new_product
        self._egg_info_path = None
    ...
    def update_editable(self, obtain=True):
        if not self.link:
            logger.debug(
                "Cannot update repository at %s; repository product is "
                "unknown",
                self.source_dir,
            )
            return
    ...
    def uninstall(self, auto_confirm=False):
        ...
        dist_path = normalize_path(dist.product)
        ...
        if (egg_info_exists and dist.egg_info.endswith('.egg-info') and
            ...
            # of pip
                ...
                for top_level_pkg in [
                    ...
                    path = os.path.join(dist.product, top_level_pkg)
                    paths_to_remove.add(path)
        ...
        elif dist.product.endswith('.egg'):
            # package installed by easy_install
            # We cannot match on dist.egg_name because it can slightly vary
            # i.e. setuptools-0.6c11-py2.6.egg vs setuptools-0.6rc11-py2.6.egg
            paths_to_remove.add(dist.product)
            easy_install_egg = os.path.split(dist.product)[1]
            easy_install_pth = os.path.join(os.path.dirname(dist.product),
                                            'easy-install.pth')
            ...
            assert (link_pointer == dist.product), (
                'Egg-link %s does not match installed product of %s '
                '(at %s)' % (link_pointer, self.name, dist.product)
            )
            paths_to_remove.add(develop_egg_link)
            easy_install_pth = os.path.join(os.path.dirname(develop_egg_link),
                                            'easy-install.pth')
            paths_to_remove.add_pth(easy_install_pth, dist.product)
    ...
    def install(self, install_options, global_options=[], root=None,
        ...
        temp_product = tempfile.mkdtemp('-record', 'pip-')
        record_filename = os.path.join(temp_product, 'install-record.txt')
        try:
    ...
    def ensure_has_source_dir(self, parent_dir):
        ...
        if self.source_dir is None:
            self.source_dir = self.build_product(parent_dir)
        return self.source_dir
            ...
            if self.use_user_site:
                if dist_in_usersite(existing_dist):
                    ...
                    raise InstallationError(
                        ...
                        (existing_dist.project_name, existing_dist.product)
                    )
            else:
                self.conflicts_with = existing_dist
        return True
